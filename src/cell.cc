...
bool voronoicell_base::nplane(vc_class &vc,double x,double y,double z,double rsq,int p_id) {
    ...
    try {
        ...
        // only to avoid warning of unreference local variable 'except'
#ifdef _MSC_VER
#include <crtdbg.h>
#else
...
// 11/21/2015 Liang Mi  --begin-------
void voronoicell_base::draw_pov_mesh_simple(double x, double y, double z, double r, FILE *fp) {
    int i, j, k, l, m, n;
    double *ptsp = pts;
    fprintf(fp, "radius %g\n", r);
    fprintf(fp, "# of vertices %d\n", p);
    fprintf(fp, "# of faces %d\n", (p - 2) << 1);
    for (i = 0; i<p; i++, ptsp += 3) 
        fprintf(fp, "%g %g %g\n", x + *ptsp*0.5, y + ptsp[1] * 0.5, z + ptsp[2] * 0.5);
    for (i = 1; i<p; i++) for (j = 0; j<nu[i]; j++) {
        k = ed[i][j];
...
void voronoicell_base::output_pov_mesh(double x, double y, double z, double** v, int** f) {
    int i, j, k, l, m, n;
    double *ptsp = pts;
    for (i = 0; i < p; i++, ptsp += 3){
        v[i][0] = x + *ptsp*0.5; v[i][1] = y + ptsp[1] * 0.5; v[i][2] = z + ptsp[2] * 0.5;
    }
    int numf = 0;
    for (i = 1; i<p; i++) for (j = 0; j<nu[i]; j++) {
        k = ed[i][j];
        if (k >= 0) {
            ...
            while (m != i) {
                n = cycle_up(ed[k][nu[k] + l], m);
                f[numf][0] = i; f[numf][1] = k; f[numf++][2] = m;
                k = m; l = n;
                m = ed[k][l]; ed[k][l] = -1 - m;
            }
        }
...
// 11/21/2015 Liang Mi  --end-------
// 09/13/2017 Liang Mi -- begin----

void voronoicell_base::output_pov_mesh_new(double x, double y, double z, 
                                           std::shared_ptr<std::vector<std::array<double,3>>> pv, 
                                           std::shared_ptr<std::vector<std::array<int, 3>>>   pf) {
    int i, j, k, l, m, n;
    double *ptsp = pts;
    for (i = 0; i < p; i++, ptsp += 3){
        std::array<double, 3> tmp = { x + *ptsp*0.5, y + ptsp[1] * 0.5, z + ptsp[2] * 0.5 };
        pv->push_back(tmp);
        //v[i][0] = x + *ptsp*0.5; v[i][1] = y + ptsp[1] * 0.5; v[i][2] = z + ptsp[2] * 0.5;
    }
    // int numf = 0;
