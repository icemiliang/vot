...
//
// Last modified: June 6th 2018, Liang Mi (icemiliang@gmail.com)
...
class container_poly : public container_base, public radius_poly {
    public:
        ...
        // Liang Mi 12/7/2017
        inline void update_h(std::vector<double> &hs) {
            c_loop_all vl(*this);
            double *pp;
            if (vl.start()) do {
                pp = p[vl.ijk] + 4 * vl.q;
                //pp[3] = pp[3];
                //pp[3] = 1;// hs[id[vl.ijk][vl.q]];
                pp[3] = hs[id[vl.ijk][vl.q]];
            } while (vl.inc());
        }
        ...
        inline void update_diagram(std::vector<vot::Dirac> &diracs, 
            ...
            voronoicell_neighbor c;
            if (vl.start()) {
                std::vector<Eigen::Triplet<double>> kijs;
                kijs.reserve(diracs.size());
                do {
                    // Update current cell
                    compute_cell(c, vl);
                    ...
                    // Coordinates of the current Centroid
                    double *pp; 
                    pp = p[vl.ijk] + ps*vl.q;
                    ...
                    ASSERT_VOT(nids.size() == face_areas.size(), 
                           "Number of neighbor ids not equal to number of neighbor faces.");

                   for (int i = 0; i < (int)nids.size(); i++) {
                       if (nids[i] < 0) continue;
                       vot::Point tmp = vot::Point(diracs[nids[i]].x() - pp[0], 
                                                   diracs[nids[i]].y() - pp[1], 
                                                   diracs[nids[i]].z() - pp[2]);
                       double dist = tmp.norm();
                       ASSERT_VOT(dist > vot::otTOLERANCE, "Distance equals zero.");
                       kijs.push_back(Eigen::Triplet<double>(idx, nids[i], -face_areas[i] / dist));
                   }
                } while (vl.inc());

                // Put kijs into Hessian
                pHessian.setFromTriplets(kijs.begin(), kijs.end());
                // Fill up diagonal
               for (int iCol = 0; iCol < (int)diracs.size(); iCol++){
                   double sum = pHessian.col(iCol).sum();
                   pHessian.coeffRef(iCol,iCol) = -sum;
                }
                // Hessian may not be symmetric because kij may not equal kji.
                // Face areas at the two sizes of a same plane, computed by voro++, could be different .
                // Need average.
                Eigen::SparseMatrix<double> tmp = pHessian.transpose();
                pHessian = pHessian + tmp;
                pHessian = pHessian * 0.5;
            }
        }
        ...
        inline void draw_cells_separately(std::string filenamePrefix, double scale) {
            ...
            if (vl.start()) do if (compute_cell(c, vl)) {
                int idx = id[vl.ijk][vl.q];
                std::string filename = filenamePrefix + std::to_string(idx) + ".gnu";
                pp = p[vl.ijk] + ps*vl.q;
                FILE *fp = safe_fopen(filename.c_str(), "w");
                c.draw_gnuplot((*pp)*scale, pp[1] * scale, pp[2] * scale, fp);
                fclose(fp);
            } while (vl.inc());
        }

        inline void draw_diagram_gnuplot(const char *filename) {
            FILE *fp = safe_fopen(filename, "w");
            draw_diagram_gnuplot(fp);
            fclose(fp);
        }
        inline void draw_diagram_gnuplot(FILE *fp = stdout) {
            c_loop_all vl(*this);
            draw_diagram_gnuplot(vl, fp);
        }
        template<class c_loop>
        void draw_diagram_gnuplot(c_loop &vl, FILE *fp) {
            voronoicell c; double *pp;
            if (vl.start()) do if (compute_cell(c, vl)) {
                pp = p[vl.ijk] + ps*vl.q;
                c.draw_gnuplot(*pp, pp[1], pp[2], fp);
