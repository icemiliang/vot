...
class voronoicell_base {
    public:
        ...
        void output_pov_mesh(double x, double y, double z, double** v, int** f);
        void output_pov_mesh_new(double x, double y, double z,
            std::shared_ptr<std::vector<std::array<double, 3>>> pv,
            std::shared_ptr<std::vector<std::array<int, 3>>>   pf);
        void draw_pov_mesh_simple(double x, double y, double z, double v, FILE *fp = stdout);
